...
This build extends the original Day 1 user booking flow and Day 2 Doctor Portal without removing those flows.

### Added
- Doctor appointment lifecycle: Pending, Confirmed, Upcoming, Completed, Cancelled, Missed.
- Doctor Calendar with Day / Week / Month controls, availability display and drag-and-drop rescheduling.
- Double-booking and unavailable-slot protection during rescheduling.
- Appointment search and date/status filtering with detail actions.
- User **My Appointments** with Upcoming, Completed, Cancelled and Missed views.
- Completed appointment actions: prescription status, view/download prescription, review doctor and rebook.
- Appointment notifications for booking, confirmation, rescheduling, cancellation, completion, missed visits and prescriptions.
- Persistent browser-side data storage using `localStorage`, so registrations, slots, bookings, status changes, prescriptions, reviews and notifications remain after refresh in the same browser.
